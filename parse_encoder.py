...
def parse_text(file_string):
    ...
    for pattern, symbol_type in symbol_searches:
        symbols += [(match.start(), match.end(), match.group(), symbol_type) for match in re.finditer(pattern, file_string)]
...
def process_comments(file_string):
    comment_symbols = []
    comment_symbols += [(match.start(), match.end(), match.group(), FULL_LINE_COMMENT_TYPE) for match in re.finditer(r"^[ \t]*#.*$", file_string, re.MULTILINE)]

    lookup = set()
    for fulline_comment in comment_symbols:
        lookup.add(fulline_comment[1])

    comment_symbols += [(match.start(), match.end(), match.group(), PART_LINE_COMMENT_TYPE) for match in re.finditer(r"#.*$", file_string, re.MULTILINE) if match.end() not in lookup]
